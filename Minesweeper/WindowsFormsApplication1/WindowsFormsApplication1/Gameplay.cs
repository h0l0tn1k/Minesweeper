...
<<<<<<< HEAD
                ...
                CustomMessageBox mbox = new CustomMessageBox();
                mbox.Label1 = "Level " +(level+1).ToString() +" completed!";
...
>>>>>>> origin/master
        ...
        private bool reveal(int x,int y)
        {
            ...
            if (battlefield.Field[y, x] == Field.Nothing  || battlefield.Field[y,x] == Field.Empty
                ||battlefield.Field[y,x] == Field.Hearth|| battlefield.Field[y, x] == Field.Gold)
            {
                visitedRectangles[y, x] = true;
            }
            else
